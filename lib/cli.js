  ...
  run() {
    return inquirer
      ...
      switch (shapeType){
        case 'Circle':
          if(text>3){
            throw new error('logo name must be less than 3 characters')
          }
      ...
      const svgLogo=new SVG(text,textColor,shapeName).render();      
        return writeFile(join(__dirname, '..', 'output', 'logo.svg'),svgLogo);
    })
